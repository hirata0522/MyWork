...
# pklファイルから配列を読み込みます
with open('Acc_artificial_untarget_'+str(group[idx])+'_worst_class_fix.pkl', 'rb') as f:
    Acc = pickle.load(f)
...
plt.title("Untarget Attack (Artificial Dataset)\nMalicious Clients : "+str(client[idx]))
...
txt="Acc_artificial_untarget_"+str(group[idx])+"_worst_class_fix.png"
plt.savefig(txt)
txt="Acc_artificial_untarget_"+str(group[idx])+"_worst_class_fix.eps"
plt.savefig(txt)
plt.clf()
plt.close()
