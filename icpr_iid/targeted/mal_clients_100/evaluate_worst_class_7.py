...
def cal_score(N,int,Y_MNIST,models,clients,X,class_a,Y):
    NNUM=[100,300,500,700,900]
    num=NNUM[int]
    # データの準備
    ...
    for i in range(10):
        ...
        if num_cs==0:
            cs[i]=False
        # print("num_cs:",i,num_cs)

        #各ローカルデータセットの損失を格納する配列に格納
        loss.append(loss_group)
        num_cs_avg+=num_cs
    # if (ami_model+mal_model)==0:
    #     print("class:",class_a,"num_cs_avg:",num_cs_avg/10,"non-atck model:",ami_model)
    # else:
    #     print("class:",class_a,"num_cs_avg:",num_cs_avg/10,"non-atck model:",ami_model/(ami_model+mal_model))
...
def eval_1(N,M,int):
    ...
    while(len(models)>0):
        ...
        for i in range(10):
            ...
            [val,idx]=cal_score(N,int,Y_MNIST_class[i],models,clients,X,i,Y)
            if val>max_val:
                max_val=val
                max_idx=idx
                max_cls=i
            class_val[i].append(val)
    ...
    with open('val_target_'+str(gr)+'_worst_class.pkl', 'wb') as f:
        pickle.dump(class_val, f)
...
#FLCertを実行する関数
def eval_class_i(N,M,int,class_i):
    NNUM=[100,300,500,700,900]
    GGROUP=[100,300,500,700,900]
    gr=GGROUP[idx]
