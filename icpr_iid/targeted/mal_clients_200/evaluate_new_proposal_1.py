...
def pred_mnist(int):
    NNUM=[200,300,500,700,900]
    GGROUP=[200,300,500,700,900]
    M=200
    gr=GGROUP[int]
    # データの準備
    mnist = keras.datasets.mnist
...
def eval_1(N,M,int):
    ...
    f = open("#Proposal_target_"+str(gr)+"_1.txt",'a')
    ...
    f.close()
    # Y_MNISTを保存します
    with open('delete_target_'+str(gr)+'_1.pkl', 'wb') as f:
...
def pred_test(int):
    ...
    # Y_MNISTを保存します
    with open('Test_target_'+str(gr)+'_1.pkl', 'wb') as f:
...
def acc_u(idx):
    ...
    with open('Test_target_'+str(gr)+'_1.pkl', 'rb') as f:
        Pred = pickle.load(f) 
    with open('Test_answer.pkl', 'rb') as f:
        Ans = pickle.load(f) 
    with open('delete_target_'+str(gr)+'_1.pkl', 'rb') as f:
...
def acc_t(idx):
    ...
    f =open('FLCert_target_'+str(gr)+'_1.txt','a')
    nn=0
    # print(delete_models)
    ...
    f.close() 
    # Y_MNISTを保存します
    with open('Acc_target_'+str(gr)+'_1.pkl', 'wb') as FFF:
        pickle.dump(res, FFF) 
