...
def cal_E_no_target_class(p,M):
    return ((1-p)-(1-p)**M)/cal_p_Sn(p,p,M,0)

def cal_E_mal(M,m_mal,p_ami,p_mal):
    return ((1-p_mal)-(1-p_ami)**(M-m_mal)*(1-p_mal)**m_mal)/cal_p_Sn(p_ami,p_mal,M,m_mal)
...
for i in range(N+1):
    ...
    for j in range(M+1):
        ...
        if (N-i)==0:
            ...
            sum_p+=0
        elif i/(N-i)<cal_E_mal(M,j,p_ami,p_mal)-cal_E_no_target_class(p_ami,M) and i/(N-i)<cal_E_E_mal(M,j,p_ami,p_mal):
            sum_p+=pp[j]
    if i==0:
        pro_1.append(1)
    else:
...
txt="Area_ieice_untarget_target.png"
plt.savefig(txt)

txt="Area_ieice_untarget_target.eps"
plt.savefig(txt)
plt.clf()
plt.close()
