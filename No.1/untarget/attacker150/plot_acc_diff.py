...
# pklファイルから配列を読み込みます
with open('Acc_untarget_'+str(group[idx])+'_1.pkl', 'rb') as f:
    Acc_1 = pickle.load(f)
with open('Acc_untarget_'+str(group[idx])+'.pkl', 'rb') as f:
    Acc_0 = pickle.load(f)

Acc=[]
for i in range(len(Acc_0)):
    Acc.append(Acc_0[i]-Acc_1[i])
# Acc_0-Acc_1
plt.clf()
fig, ax = plt.subplots()
A=[]
...
plt.title("Untarget Attack Proposal-1\nMalicious Clients : "+str(client[idx]))
...
txt="Acc_untarget_"+str(group[idx])+"_diff.png"
plt.savefig(txt)
plt.clf()
plt.close()
