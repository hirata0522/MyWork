...
def pred_mnist(int):
    NNUM=[57,91,130,179,243]
    GGROUP=[150,125,100,75,50]
    M=200
    gr=GGROUP[int]
    # データの準備
    mnist = keras.datasets.mnist
...
def eval_1(N,M,int):
    ...
    f = open("#Proposal_untarget_"+str(gr)+"_1.txt",'a')
    ...
    f.close()
    # Y_MNISTを保存します
    with open('delete_untarget_'+str(gr)+'_1.pkl', 'wb') as f:
...
def pred_test(int):
    ...
    # Y_MNISTを保存します
    with open('Test_untarget_'+str(gr)+'_1.pkl', 'wb') as f:
...
def acc_u(idx):
    ...
    with open('Test_untarget_'+str(gr)+'_1.pkl', 'rb') as f:
        Pred = pickle.load(f) 
    with open('Test_answer.pkl', 'rb') as f:
        Ans = pickle.load(f) 
    with open('delete_untarget_'+str(gr)+'_1.pkl', 'rb') as f:
    ...
    f =open('FLCert_untarget_'+str(gr)+'_1.txt','a')
    nn=0
    # print(delete_models)
    res=[]
    ...
    f.close()
    # Y_MNISTを保存します
    with open('Acc_untarget_'+str(gr)+'_1.pkl', 'wb') as FFF:
        pickle.dump(res, FFF)
...
def search_level(py, pz, k, n):
    # 不等式(6)を満たす最大の整数mを探す
    # 二分探索で効率的に探す
    m=0
    m_max=0
    while m <= n-k:
        if np.ceil(py * math.comb(n,k))/math.comb(n,k) - np.floor(pz * math.comb(n,k))/math.comb(n,k) > 2 - 2 * math.comb(n-m,k) / math.comb(n,k):
            m_max=m
            m+=1
        else:
            m+=1
...
def algorithm1(models, k, N, alpha,idx):
    # modelsはモデルの格納された配列
    # kはグループのサイズ
    # Nはモデル総数
    # αはCAのパラメータ
    ...
    # 
    # アルゴリズム1の実装
    # Dの各テスト入力の予測ラベルと証明されたセキュリティレベルをリストとして返す
    predicted_labels = []
    certified_levels = []
    # N個のグローバルモデルを学習する
    with open('Test_untarget_'+str(gr)+'_1.pkl', 'rb') as ff:
        Pred = pickle.load(ff)
    
    # f =open('CA_target_'+str(gr)+'_1.txt','a')
    
    cnt=0
    # Dの各テスト入力に対して処理する
    for i in range(len(Pred)):
        # N個のグローバルモデルの予測ラベルの頻度を数える
        counts = np.zeros(10) # Lはラベルの種類数
        for j in range(len(models)):
            counts[Pred[i][models[j]]] += 1
        # 最も頻度の高いラベルを予測ラベルとする
        y_hat = np.argmax(counts)
        # ラベル確率の下限と上限を求める
        py = beta.ppf(alpha / len(Pred), counts[y_hat], N - counts[y_hat] + 1)
        # print(py)
        pz = 1 - py
        # 証明されたセキュリティレベルを求める
        if py > pz:
            m_star = search_level(py, pz, k, 1000)
            if m_star>NNUM[idx]:
                cnt+=1
        else:
            y_hat = "ABSTAIN"
            m_star = "ABSTAIN"
        # 予測ラベルと証明されたセキュリティレベルをリストに追加する
        predicted_labels.append(y_hat)
        certified_levels.append(m_star)

    # f =open('CA_target_'+str(gr)+'_1.txt','a')
    print(certified_levels)
    print(cnt/len(Pred))
    certified_levels=np.array(certified_levels)
    print(np.mean(certified_levels))

    with open('CA_untarget_'+str(gr)+'_1.pkl', 'wb') as FFF:
        pickle.dump(certified_levels, FFF) 
...
algorithm1(models, k, M, 0.001,idx)