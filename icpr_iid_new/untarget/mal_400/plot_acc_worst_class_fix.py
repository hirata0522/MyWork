...
# pklファイルから配列を読み込みます
with open('Acc_untarget_'+str(group[idx])+'_worst_class_fix.pkl', 'rb') as f:
    Acc = pickle.load(f)
...
txt="Acc_untarget_"+str(group[idx])+"_worst_class_fix.png"
plt.savefig(txt)
txt="Acc_untarget_"+str(group[idx])+"_worst_class_fix.eps"
plt.savefig(txt)
plt.clf()
plt.close()
